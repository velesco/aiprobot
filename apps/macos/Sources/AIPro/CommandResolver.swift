...
enum CommandResolver {
    private static let projectRootDefaultsKey = "aipro.gatewayProjectRootPath"
    private static let helperName = "aipro"
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/aipro")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let aiproPaths = self.aiproManagedPaths(home: home)
        if !aiproPaths.isEmpty {
            extras.insert(contentsOf: aiproPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + aiproPaths.count)
        var seen = Set<String>()
    ...
    static func aiproExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectAIProExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func hasAnyAIProInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.aiproExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let aiproPath = self.projectAIProExecutable(projectRoot: root) {
                return [aiproPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "aipro", subcommand] + extraArgs
            }
            if let aiproPath = self.aiproExecutable(searchPaths: searchPaths) {
                return [aiproPath, subcommand] + extraArgs
            }
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real aipro CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/aipro"
        ...
        \(cliSection)
        \(projectSection)
        if command -v aipro >/dev/null 2>&1; then
          CLI="$(command -v aipro)"
          aipro \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/aipro.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/aipro.js"
            node "$PRJ/bin/aipro.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent aipro"
          pnpm --silent aipro \(quotedArgs);
        else
          echo "aipro CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? AIProConfigFile.loadDict()
