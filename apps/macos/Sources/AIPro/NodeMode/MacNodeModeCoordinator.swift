...
final class MacNodeModeCoordinator {
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: AIProNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [AIProCapability.canvas.rawValue, AIProCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(AIProCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if AIProLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(AIProCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        ...
        if capsSet.contains(AIProCapability.camera.rawValue) {
        ...
        if capsSet.contains(AIProCapability.location.rawValue) {
