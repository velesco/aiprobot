...
final class CanvasWindowController: NSWindowController, WKNavigationDelegate, NSWindowDelegate {
          ...
          try {
            ...
            globalThis.addEventListener('a2uiaction', (evt) => {
              try {
                ...
                const handler = globalThis.webkit?.messageHandlers?.aiproCanvasA2UIAction;
                ...
                location.href = 'aipro://agent?' + params.toString();
              } catch {}
            }, true);
          } catch {}
        })();
    ...
    func snapshot(to outPath: String?) async throws -> String {
        ...
        if let outPath, !outPath.isEmpty {
            ...
            path = "/tmp/aipro-canvas-\(CanvasWindowController.sanitizeSessionKey(self.sessionKey))-\(ts).png"
        }
