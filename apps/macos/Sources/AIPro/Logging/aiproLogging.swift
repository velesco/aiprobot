...
enum AIProLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = AIProOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = AIProFileLogHandler(label: label)
...
<<<<<<<< HEAD:apps/macos/Sources/AIPro/Logging/DigibossLogging.swift
            return ("ro.aipro", label)
========
            return ("ai.aipro", label)
>>>>>>>> upstream-sync:apps/macos/Sources/AIPro/Logging/aiproLogging.swift
        }
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        AIProLogging.bootstrapIfNeeded()
        let label = AIProLogging.makeLabel(subsystem: subsystem, category: category)
...
struct AIProOSLogHandler: LogHandler {
...
struct AIProFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = AIProLogging.parseLabel(self.label)
