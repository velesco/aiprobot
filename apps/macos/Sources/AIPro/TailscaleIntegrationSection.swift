...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.aipro/aipro.json. Prefer AIPRO_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.aipro/aipro.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.aipro/aipro.json. Restart the gateway to apply."
        }
