...
struct MacGatewayChatTransport: AIProChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> AIProChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(AIProChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> AIProChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                AIProGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<AIProChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any AIProChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = AIProChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: AIProChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<AIProChatView>) -> NSViewController
    {
