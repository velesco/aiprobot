...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: AIProOAuthStore.AnthropicOAuthStatus = AIProOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: AIProOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = AIProOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = AIProOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try AIProOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. AIPro can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: AIProOAuthStore.AnthropicOAuthStatus,
