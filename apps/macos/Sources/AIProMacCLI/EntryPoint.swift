...
struct AIProMacCLI {
    static func main() async {
        ...
        default:
            fputs("aipro-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      aipro-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      aipro-mac discover [--timeout <ms>] [--json] [--include-local]
      aipro-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      aipro-mac connect
      aipro-mac connect --url ws://127.0.0.1:18789 --json
      aipro-mac discover --timeout 3000 --json
      aipro-mac wizard --mode local
    """)
}
