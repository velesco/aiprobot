...
struct IOSGatewayChatTransport: AIProChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> AIProChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(AIProChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> AIProChatHistoryPayload {
        ...
        return try JSONDecoder().decode(AIProChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [AIProChatAttachmentPayload]) async throws -> AIProChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [AIProChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(AIProChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(AIProGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AIProGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AIProChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: AIProAgentEventPayload.self)
                        {
