...
@MainActor
struct AIProChatComposer: View {
    @Bindable var viewModel: AIProChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(AIProChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(AIProChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \AIProPendingAttachment.id)
                { (att: AIProPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            AIProPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(AIProChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(AIProChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(AIProChatTheme.subtleCard)
        .clipShape(Capsule())
    }
