...
@MainActor
@Observable
public final class AIProChatViewModel {
    public private(set) var messages: [AIProChatMessage] = []
    ...
    public var attachments: [AIProPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [AIProChatPendingToolCall] = []
    public private(set) var sessions: [AIProChatSessionEntry] = []
    private let transport: any AIProChatTransport
    ...
    private var pendingToolCallsById: [String: AIProChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any AIProChatTransport) {
    ...
    public var sessionChoices: [AIProChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [AIProChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [AIProChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: AIProPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [AIProChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: AIProChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [AIProChatMessage]) -> [AIProChatMessage] {
        var result: [AIProChatMessage] = []
    ...
    private static func dedupeKey(for message: AIProChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [AIProChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> AIProChatAttachmentPayload in
            AIProChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> AIProChatSessionEntry {
        AIProChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: AIProChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: AIProChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: AIProAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = AIProChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> AIProPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
