...
private final class TestChatTransport: @unchecked Sendable, AIProChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [AIProChatHistoryPayload]
    private let sessionsResponses: [AIProChatSessionsListResponse]

    private let stream: AsyncStream<AIProChatTransportEvent>
    private let continuation: AsyncStream<AIProChatTransportEvent>.Continuation

    init(
        historyResponses: [AIProChatHistoryPayload],
        sessionsResponses: [AIProChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<AIProChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> AIProChatHistoryPayload {
        ...
        return self.historyResponses.last ?? AIProChatHistoryPayload(
        ...
        attachments _: [AIProChatAttachmentPayload]) async throws -> AIProChatSendResponse
    {
        ...
        return AIProChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> AIProChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? AIProChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: AIProChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = AIProChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = AIProChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { AIProChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = AIProChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = AIProChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = AIProChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { AIProChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
