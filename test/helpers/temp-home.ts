...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("AIPRO_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.AIPRO_STATE_DIR = path.join(base, ".aipro");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "aipro-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".aipro", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
