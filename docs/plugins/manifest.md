...
read_when:
  - You are building a AIPro plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---
# Plugin manifest (aipro.plugin.json)

Every plugin **must** ship a `aipro.plugin.json` file in the **plugin root**.
AIPro uses this manifest to validate configuration **without executing plugin
