...
If the file is missing, AIPro uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/clawd`). You usually only need a config to:
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "aipro", "clawd"] } }
    ]
  }
}
```
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `aipro onboard --auth-choice kimi-code-api-key`.
...
AIPro can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for clawd and expose a small loopback control service.
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__aipro/ws`
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_aipro-bridge._tcp` under `~/.aipro/dns/` using the standard discovery domain `aipro.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `aipro.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `aipro.internal` via that server
