...
- During heartbeat runs, AIPro treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `aipro sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, AIPro skips the heartbeat run entirely (no model call).
...
```bash
aipro system event --text "Check for urgent follow-ups" --mode now
```
