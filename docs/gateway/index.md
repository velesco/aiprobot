...
- Gateway auth is required by default: set `gateway.auth.token` (or `AIPRO_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
...
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `aipro gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.aipro.*` is cleaned up).
  - `aipro doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are AIPro profile services.
  ...
  - Cleanup: `aipro gateway uninstall` (current service) and `aipro doctor` (legacy migrations).
...
- To restart, use `aipro gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `aipro gateway install` first.
...
`aipro gateway install` writes the user unit. `aipro doctor` audits the
...
- `aipro gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `aipro gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
