...
Only the Gateway advertises `_aipro-gw._tcp`.

## Service types

- `_aipro-gw._tcp` — gateway transport beacon (used by macOS/iOS/Android nodes).
...
- `transport=gateway`
- `cliPath=<path>` (optional; absolute path to a runnable `aipro` entrypoint)
