...
AIPro can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere AIPro can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, AIPro defaults
...
TTS config lives under `messages.tts` in `aipro.json`.
...
- If `provider` is **unset**, AIPro prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.aipro/settings/tts.json`, override with `AIPRO_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, AIPro retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so AIPro registers
