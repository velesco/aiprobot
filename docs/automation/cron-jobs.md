...
- Jobs persist under `~/.aipro/cron/` so restarts don’t lose schedules.
...
aipro cron list
aipro cron run <job-id> --force
aipro cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.aipro/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `aipro cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, AIPro defaults to `announce`.
...
- Job store: `~/.aipro/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.aipro/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
aipro cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
aipro cron edit <jobId> --agent ops
aipro cron edit <jobId> --clear-agent
```
...
```bash
aipro system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`aipro system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `AIPRO_SKIP_CRON`.
