...
This document explains how AIPro manages sessions end-to-end:
...
AIPro is designed around a single **Gateway process** that owns session state.
...
AIPro persists sessions in two layers:
...
- Transcripts: `~/.aipro/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

AIPro resolves these via `src/config/sessions.ts`.
...
AIPro intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (AIPro consumes the events, but Pi decides when to compact).
...
AIPro also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, AIPro bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, AIPro leaves it alone.
...
AIPro supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- AIPro strips/suppresses this in the delivery layer.

As of `2026.1.10`, AIPro also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
AIPro uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but AIPro’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `aipro status`.
