...
AIPro integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- AIPro owns lifecycle when `channels.signal.autoStart=true`.
...
- AIPro spawns `imsg rpc` as a child process (legacy iMessage integration).
