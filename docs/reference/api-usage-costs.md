...
AIPro features that can generate provider usage or paid API calls.
...
- `aipro status --usage` and `aipro channels list` show provider **usage windows**
...
AIPro can pick up credentials from:
...
### 8) Model scan / probe
`aipro models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
