...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects AIPro to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
If you just installed AIPro, run the wizard:
...
2. Configuring app credentials in AIPro
...
1. You already ran `aipro channels add` for Feishu
2. The gateway is running (`aipro gateway status`)
...
```bash
aipro pairing approve feishu <CODE>
```
...
2. Run `aipro logs --follow` and look for `chat_id`
...
2. Run `aipro logs --follow` and look for `open_id`
...
| Command                 | Description                   |
| ----------------------- | ----------------------------- |
| `aipro gateway status`  | Show gateway status           |
| `aipro gateway install` | Install/start gateway service |
| `aipro gateway stop`    | Stop gateway service          |
| `aipro gateway restart` | Restart gateway service       |
| `aipro logs --follow`   | Tail gateway logs             |
...
4. Check logs: `aipro logs --follow`
...
5. Ensure the gateway is running: `aipro gateway status`
6. Check logs: `aipro logs --follow`
