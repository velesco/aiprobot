...
Matrix is an open, decentralized messaging protocol. AIPro connects as a Matrix **user**
...
```bash
aipro plugins install @aipro/matrix
```

Local checkout (when running from a git repo):

```bash
aipro plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `aipro plugins install @aipro/matrix`
   - From a local checkout: `aipro plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: AIPro calls the same
     login endpoint, stores the access token in `~/.aipro/credentials/matrix/credentials.json`,
...
- On first connection, AIPro requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  AIPro logs a warning.
...
`~/.aipro/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `aipro pairing list matrix`
  - `aipro pairing approve matrix <CODE>`
...
- On startup, AIPro resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
