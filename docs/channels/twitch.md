...
read_when:
  - Setting up Twitch chat integration for AIPro
---
# Twitch (plugin)

Twitch chat support via IRC connection. AIPro connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
aipro plugins install @aipro/twitch
```

Local checkout (when running from a git repo):

```bash
aipro plugins install ./extensions/twitch
```
...
4) Configure the token:
   - Env: `AIPRO_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "aipro",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use AIPRO_TWITCH_ACCESS_TOKEN env var)
...
```bash
AIPRO_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
