...
Tlon is a decentralized messenger built on Urbit. AIPro connects to your Urbit ship and can
...
```bash
aipro plugins install @aipro/tlon
```

Local checkout (when running from a git repo):

```bash
aipro plugins install ./extensions/tlon
```
...
Use these with `aipro message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, AIPro replies in-thread.
