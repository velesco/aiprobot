...
- Talk to AIPro via Discord DMs or guild channels.
...
3. Configure AIPro with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `aipro pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running AIPro in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents AIPro needs
...
Discord uses numeric ids everywhere; AIPro config prefers ids.
...
- Owner hint: when a per-guild or per-channel `users` allowlist matches the sender, AIPro treats that sender as the owner in the system prompt. For a global owner across channels, set `commands.ownerAllowFrom`.
...
- First: run `aipro doctor` and `aipro channels status --probe` (actionable warnings + quick audits).
...
When enabled, AIPro uses the member identity for allowlists and labels the
...
- On startup, AIPro resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:

- The registered commands mirror AIPro’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; AIPro enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`aipro gateway --force`) after confirming no other processes own the Discord session.
