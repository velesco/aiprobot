...
read_when:
  - You want AIPro to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables AIPro to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`aipro onboard`) and `aipro channels add` list optional channel plugins.
...
```bash
aipro plugins install @aipro/nostr
```

Use a local checkout (dev workflows):

```bash
aipro plugins install --link <path-to-aipro>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "aipro@example.com",
        "lud16": "aipro@example.com"
      }
    }
  }
}
