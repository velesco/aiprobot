...
你可以在 `~/.aipro/aipro.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/aipro.log"
  }
}
```
...
```bash
aipro channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.aipro/aipro.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/aipro/aipro-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- AIPro 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
AIPRO_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `aipro plugins enable diagnostics-otel` 启用插件。
...
- `aipro.tokens`（计数器，属性：`aipro.token`、`aipro.channel`、`aipro.provider`、`aipro.model`）
- `aipro.cost.usd`（计数器，属性：`aipro.channel`、`aipro.provider`、`aipro.model`）
- `aipro.run.duration_ms`（直方图，属性：`aipro.channel`、`aipro.provider`、`aipro.model`）
- `aipro.context.tokens`（直方图，属性：`aipro.context`、`aipro.channel`、`aipro.provider`、`aipro.model`）

消息流：

- `aipro.webhook.received`（计数器，属性：`aipro.channel`、`aipro.webhook`）
- `aipro.webhook.error`（计数器，属性：`aipro.channel`、`aipro.webhook`）
- `aipro.webhook.duration_ms`（直方图，属性：`aipro.channel`、`aipro.webhook`）
- `aipro.message.queued`（计数器，属性：`aipro.channel`、`aipro.source`）
- `aipro.message.processed`（计数器，属性：`aipro.channel`、`aipro.outcome`）
- `aipro.message.duration_ms`（直方图，属性：`aipro.channel`、`aipro.outcome`）

队列 + 会话：

- `aipro.queue.lane.enqueue`（计数器，属性：`aipro.lane`）
- `aipro.queue.lane.dequeue`（计数器，属性：`aipro.lane`）
- `aipro.queue.depth`（直方图，属性：`aipro.lane` 或 `aipro.channel=heartbeat`）
- `aipro.queue.wait_ms`（直方图，属性：`aipro.lane`）
- `aipro.session.state`（计数器，属性：`aipro.state`、`aipro.reason`）
- `aipro.session.stuck`（计数器，属性：`aipro.state`）
- `aipro.session.stuck_age_ms`（直方图，属性：`aipro.state`）
- `aipro.run.attempt`（计数器，属性：`aipro.attempt`）

### 导出的 span（名称 + 关键属性）

- `aipro.model.usage`
  - `aipro.channel`、`aipro.provider`、`aipro.model`
  - `aipro.sessionKey`、`aipro.sessionId`
  - `aipro.tokens.*`（input/output/cache_read/cache_write/total）
- `aipro.webhook.processed`
  - `aipro.channel`、`aipro.webhook`、`aipro.chatId`
- `aipro.webhook.error`
  - `aipro.channel`、`aipro.webhook`、`aipro.chatId`、`aipro.error`
- `aipro.message.processed`
  - `aipro.channel`、`aipro.outcome`、`aipro.chatId`、`aipro.messageId`、`aipro.sessionKey`、`aipro.sessionId`、`aipro.reason`
- `aipro.session.stuck`
  - `aipro.state`、`aipro.ageMs`、`aipro.queueDepth`、`aipro.sessionKey`、`aipro.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `aipro doctor`。
