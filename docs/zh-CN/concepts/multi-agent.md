...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.aipro/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.aipro/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 状态目录：`~/.aipro`（或 `AIPRO_STATE_DIR`）
- 工作区：`~/.aipro/workspace`（或 `~/.aipro/workspace-<agentId>`）
- 智能体目录：`~/.aipro/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.aipro/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你什么都不做，AIPro 运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.aipro/workspace`（或当设置了 `AIPRO_PROFILE` 时为 `~/.aipro/workspace-<profile>`）。
- 状态默认为 `~/.aipro/agents/main/agent`。
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.aipro/workspace-alex" },
      { id: "mia", workspace: "~/.aipro/workspace-mia" },
    ],
  },
  bindings: [
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.aipro/workspace-home",
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.aipro/credentials/whatsapp/personal
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.aipro/workspace-chat",
        model: "anthropic/claude-sonnet-4-5",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.aipro/workspace-opus",
