...
summary: 修复 Linux 上 AIPro 浏览器控制的 Chrome/Brave/Edge/Chromium CDP 启动问题
title: 浏览器故障排除
x-i18n:
...
AIPro 的浏览器控制服务器无法启动 Chrome/Brave/Edge/Chromium，出现以下错误：
...
在 Ubuntu（和许多 Linux 发行版）上，默认的 Chromium 安装是 **snap 包**。Snap 的 AppArmor 限制会干扰 AIPro 启动和监控浏览器进程的方式。
...
然后更新你的 AIPro 配置（`~/.aipro/aipro.json`）：
...
如果你必须使用 snap Chromium，配置 AIPro 附加到手动启动的浏览器：
...
启用：`systemctl --user enable --now aipro-browser.service`
...
你正在使用 `chrome` 配置文件（扩展中继）。它期望 AIPro 浏览器扩展附加到一个活动标签页。

修复选项：

1. **使用托管浏览器：** `aipro browser start --browser-profile aipro`
   （或设置 `browser.defaultProfile: "aipro"`）。
2. **使用扩展中继：** 安装扩展，打开一个标签页，然后点击 AIPro 扩展图标来附加它。

注意事项：

- `chrome` 配置文件在可能时使用你的**系统默认 Chromium 浏览器**。
- 本地 `aipro` 配置文件自动分配 `cdpPort`/`cdpUrl`；仅为远程 CDP 设置这些。
