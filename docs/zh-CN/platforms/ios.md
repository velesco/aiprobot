...
- 网络路径：
  ...
  - 通过单播 DNS-SD 的 Tailnet（示例域：`aipro.internal.`），**或**
  - 手动主机/端口（备选）。
...
```bash
aipro nodes status
aipro gateway call node.list --params "{}"
```
...
Gateway 网关在 `local.` 上广播 `_aipro-gw._tcp`。iOS 应用会自动列出这些。

### Tailnet（跨网络）

如果 mDNS 被阻止，使用单播 DNS-SD 区域（选择一个域；示例：`aipro.internal.`）和 Tailscale 分割 DNS。
...
```bash
aipro nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18793/__aipro__/canvas/"}'
```

注意事项：

- Gateway 网关 canvas 主机服务于 `/__aipro__/canvas/` 和 `/__aipro__/a2ui/`。
...
```bash
aipro nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__aipro; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
aipro nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- 配对提示从未出现：运行 `aipro nodes pending` 并手动批准。
