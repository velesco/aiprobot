---
read_when:
  - 在 DigitalOcean 上设置 AIPro
  - 寻找便宜的 VPS 托管来运行 AIPro
summary: 在 DigitalOcean 上运行 AIPro（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 AIPro

## 目标

以 **$6/月**（或使用预留定价 $4/月）在 DigitalOcean 上运行持久的 AIPro Gateway 网关。
...
# Check service
systemctl --user status aipro-gateway.service

# View logs
journalctl --user -u aipro-gateway.service -f
```
...
- `~/.aipro/` — 配置、凭证、会话数据
- `~/.aipro/workspace/` — 工作区（SOUL.md、记忆等）
...
journalctl -u aipro --no-pager -n 50
```
