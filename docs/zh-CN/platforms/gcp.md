---
read_when:
  - 你想在 GCP 上 24/7 运行 AIPro
  - 你想要在自己的 VM 上运行生产级、常驻的 Gateway 网关
  - 你想完全控制持久化、二进制文件和重启行为
summary: 在 GCP Compute Engine VM（Docker）上 24/7 运行 AIPro Gateway 网关并持久化状态
title: GCP
x-i18n:
...
# 在 GCP Compute Engine 上运行 AIPro（Docker，生产 VPS 指南）

## 目标

使用 Docker 在 GCP Compute Engine VM 上运行持久化的 AIPro Gateway 网关，具有持久状态、内置二进制文件和安全的重启行为。

如果你想要"AIPro 24/7 大约 $5-12/月"，这是在 Google Cloud 上的可靠设置。
...
- 安装 Docker（隔离的应用运行时）
- 在 Docker 中启动 AIPro Gateway 网关
- 在主机上持久化 `~/.aipro` + `~/.aipro/workspace`（重启/重建后仍保留）
...
```bash
gcloud projects create my-aipro-project --name="AIPro Gateway"
gcloud config set project my-aipro-project
```
...
```bash
gcloud compute instances create aipro-gateway \
  --zone=us-central1-a \
...
```bash
gcloud compute ssh aipro-gateway --zone=us-central1-a
```
...
```bash
AIPRO_IMAGE=aipro:latest
AIPRO_GATEWAY_TOKEN=change-me-now
AIPRO_GATEWAY_BIND=lan
...
services:
  aipro-gateway:
    ...
    environment:
      ...
      - NODE_ENV=production
      - TERM=xterm-256color
      - AIPRO_GATEWAY_BIND=${AIPRO_GATEWAY_BIND}
      - AIPRO_GATEWAY_PORT=${AIPRO_GATEWAY_PORT}
      - AIPRO_GATEWAY_TOKEN=${AIPRO_GATEWAY_TOKEN}
    ...
    ports:
      ...
      - "127.0.0.1:${AIPRO_GATEWAY_PORT}:18789"
...
```bash
docker compose exec aipro-gateway which gog
docker compose exec aipro-gateway which goplaces
docker compose exec aipro-gateway which wacli
```
...
```bash
docker compose logs -f aipro-gateway
```
...
```bash
gcloud compute ssh aipro-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
```
...
AIPro 在 Docker 中运行，但 Docker 不是真实来源。
所有长期状态必须在重启、重建和重启后仍然存在。

| 组件             | 位置                           | 持久化机制    | 说明                        |
| ---------------- | ------------------------------ | ------------- | --------------------------- |
| Gateway 网关配置 | `/home/node/.aipro/`           | 主机卷挂载    | 包括 `aipro.json`、令牌     |
| 模型认证配置文件 | `/home/node/.aipro/`           | 主机卷挂载    | OAuth 令牌、API 密钥        |
| Skill 配置       | `/home/node/.aipro/skills/`    | 主机卷挂载    | Skill 级别状态              |
| 智能体工作区     | `/home/node/.aipro/workspace/` | 主机卷挂载    | 代码和智能体产物            |
| WhatsApp 会话    | `/home/node/.aipro/`           | 主机卷挂载    | 保留 QR 登录                |
| Gmail 密钥环     | `/home/node/.aipro/`           | 主机卷 + 密码 | 需要 `GOG_KEYRING_PASSWORD` |
...
```bash
# 首先停止 VM
gcloud compute instances stop aipro-gateway --zone=us-central1-a

# 更改机器类型
gcloud compute instances set-machine-type aipro-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# 启动 VM
gcloud compute instances start aipro-gateway --zone=us-central1-a
```
...
2. 授予 Compute Instance Admin 角色（或更窄的自定义角色）：
   ```bash
   gcloud projects add-iam-policy-binding my-aipro-project \
     --member="serviceAccount:aipro-deploy@my-aipro-project.iam.gserviceaccount.com" \
