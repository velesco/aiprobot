...
使用 Nix 运行 AIPro 的推荐方式是通过 **[nix-aipro](https://github.com/aipro/nix-aipro)** — 一个开箱即用的 Home Manager 模块。
...
> **📦 完整指南：[github.com/aipro/nix-aipro](https://github.com/aipro/nix-aipro)**
>
> nix-aipro 仓库是 Nix 安装的权威来源。本页只是一个快速概述。
...
当设置 `AIPRO_NIX_MODE=1` 时（nix-aipro 会自动设置）：

AIPro 支持 **Nix 模式**，使配置确定性并禁用自动安装流程。
通过导出以下环境变量启用：
...
AIPro 从 `AIPRO_CONFIG_PATH` 读取 JSON5 配置，并将可变数据存储在 `AIPRO_STATE_DIR` 中。

- `AIPRO_STATE_DIR`（默认：`~/.aipro`）
- `AIPRO_CONFIG_PATH`（默认：`$AIPRO_STATE_DIR/aipro.json`）
...
[`scripts/package-mac-app.sh`](https://github.com/aipro/aipro/blob/main/scripts/package-mac-app.sh) 将此模板复制到应用包中并修补动态字段（bundle ID、版本/构建号、Git SHA、Sparkle 密钥）。这使 plist 对于 SwiftPM 打包和 Nix 构建保持确定性（它们不依赖完整的 Xcode 工具链）。

## 相关内容

- [nix-aipro](https://github.com/aipro/nix-aipro) — 完整设置指南
