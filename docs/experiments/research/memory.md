...
read_when:
  - Designing workspace memory (~/.aipro/workspace) beyond daily Markdown logs
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.aipro/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.aipro/workspace` as canonical human-readable memory.
...
```
~/.aipro/workspace/.memory/index.sqlite
```
...
Pragmatic take for `~/.aipro/workspace`:
