---
summary: "Updating AIPro safely (global install or source), plus rollback strategy"
read_when:
  - Updating AIPro
...
AIPro is moving fast (pre “1.0”). Treat updates like shipping infra: update → run checks → restart (or use `aipro update`, which restarts) → verify.

## Recommended: re-run the website installer (upgrade in place)

The **preferred** update path is to re-run the installer from the website. It
detects existing installs, upgrades in place, and runs `aipro doctor` when
needed.
...
- For **global installs**, the script uses `npm install -g aipro@latest` under the hood.
...
- Installs deps, builds, builds the Control UI, and runs `aipro doctor`.
- Restarts the gateway by default (use `--no-restart` to skip).

If you installed via **npm/pnpm** (no git metadata), `aipro update` will try to update via your package manager. If it can’t detect the install, use “Update (global install)” instead.
...
- Switching between global and git installs is easy: install the other flavor, then run `aipro doctor` so the gateway service entrypoint is rewritten to the current install.

## Always Run: `aipro doctor`
...
- Detect and migrate older gateway services (launchd/systemd; legacy schtasks) to current AIPro services.
...
- Windows (WSL2): `systemctl --user restart aipro-gateway[-<profile>].service`
  - `launchctl`/`systemctl` only work if the service is installed; otherwise run `aipro gateway install`.
...
Tip: to see the current published version, run `npm view aipro version`.
