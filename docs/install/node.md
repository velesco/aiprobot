...
read_when:
  - "You installed AIPro but `aipro` is “command not found”"
...
AIPro’s runtime baseline is **Node 22+**.

If you can run `npm install -g aipro@latest` but later see `aipro: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
