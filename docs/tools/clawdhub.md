...
If you want to add new capabilities to your AIPro agent, ClawdHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
By default, the CLI installs skills into `./skills` under your current working directory. If a AIPro workspace is configured, `clawdhub` falls back to that workspace unless you override `--workdir` (or `CLAWDHUB_WORKDIR`). AIPro loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.aipro/skills` or bundled skills, workspace skills take precedence.
...
`clawdhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/aipro/skills` and `~/.aipro/skills`). This is designed to find older skill installs without extra flags.
