---
summary: "Agent tool surface for AIPro (browser, canvas, nodes, message, cron) replacing legacy `aipro-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `aipro-*` skills
title: "Tools"
---

# Tools (AIPro)

AIPro exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `aipro-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `aipro.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, AIPro logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`aipro node run`).
...
- Requires a Brave API key (recommended: `aipro configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated AIPro-managed browser.
...
- Quick smoke: `aipro nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `aipro gateway` restart in-place)
