---
title: Lobster
summary: "Typed workflow runtime for AIPro with resumable approval gates."
description: Typed workflow runtime for AIPro — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets AIPro run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: AIPro runs one Lobster tool call and gets a structured result.
...
AIPro launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In AIPro tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the AIPro Gateway (see the [Lobster repo](https://github.com/aipro/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), AIPro keeps core tools enabled. To restrict core
