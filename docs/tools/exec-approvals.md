...
- **gateway host** → `aipro` process on the gateway machine
...
`~/.aipro/exec-approvals.json` directly.

CLI: `aipro approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
