...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/aipro`)
...
If the page fails to load, start the Gateway first: `aipro gateway`.
...
you revoke it with `aipro devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.aipro/aipro.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. AIPro
...
AIPro **blocks** Control UI connections without device identity.
