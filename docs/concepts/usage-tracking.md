...
- `/usage cost` in chats: local cost summary aggregated from AIPro session logs.
- CLI: `aipro status --usage` prints a full per-provider breakdown.
- CLI: `aipro channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
