...
AIPro uses a single agent workspace directory (`agents.defaults.workspace`) as the agent’s **only** working directory (`cwd`) for tools and context.

Recommended: use `aipro setup` to create `~/.aipro/aipro.json` if missing and initialize the workspace files.
...
On the first turn of a new session, AIPro injects the contents of these files directly into the agent context.

Blank files are skipped. Large files are trimmed and truncated with a marker so prompts stay lean (read the file for full content).

If a file is missing, AIPro injects a single “missing file” marker line (and `aipro setup` will create a safe default template).
