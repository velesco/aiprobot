...
service via the external `aipro` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`AIPRO_PROFILE`; legacy `com.aipro.*` is supported).
