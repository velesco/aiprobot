---
summary: "AIPro on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up AIPro on a Raspberry Pi
  - Running AIPro on ARM devices
  - Building a cheap always-on personal AI
---

# AIPro on Raspberry Pi

## Goal

Run a persistent, always-on AIPro Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most AIPro features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled aipro
...
```bash
# Check logs
journalctl -u aipro --no-pager -n 100

# Common fix: rebuild
cd ~/aipro  # if using hackable install
npm run build
sudo systemctl restart aipro
```
