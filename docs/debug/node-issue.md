...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in AIPro
...
Running AIPro via Node with `tsx` fails at startup with:

```
[aipro] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `aipro status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch aipro.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node aipro.mjs status` works on Node 25.
