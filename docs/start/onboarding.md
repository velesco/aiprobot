---
summary: "First-run onboarding flow for AIPro (macOS app)"
read_when:
...
- Writes credentials to `~/.aipro/credentials/oauth.json`
...
The app can install the global `aipro` CLI via npm/pnpm so terminal
...
On the first agent run, AIPro bootstraps a workspace (default `~/.aipro/workspace`):
...
```bash
aipro webhooks gmail setup --account you@gmail.com
```
