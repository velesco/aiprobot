...
Remote Gateway:
```bash
aipro tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `aipro tui --deliver`
...
- Check the Gateway logs: `aipro logs --follow`.
- Confirm the agent can run: `aipro status` and `aipro models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).
- `--history-limit <n>`: History entries to load (default 200)

## Troubleshooting
- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `aipro agents list` and your routing config.
