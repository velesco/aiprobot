...
Delivery:
- Publish as `@aipro/plugin-sdk` (or export from core under `aipro/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `AIProPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: AIProConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: AIProConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: AIProConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: AIProConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: AIProConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: AIProConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@aipro/plugin-sdk`.
- Add `api.runtime` to `AIProPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `aiproRuntime: ">=2026.2.0"`).
