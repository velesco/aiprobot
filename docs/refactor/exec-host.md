...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.aipro/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.aipro/node.json` (node id + pairing token).
...
Path: `~/.aipro/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.aipro/exec-approvals.sock",
...
- Unix socket at `~/.aipro/exec-approvals.sock` (0600).
