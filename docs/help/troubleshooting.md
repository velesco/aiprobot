...
You can also set `AIPRO_VERBOSE=1` instead of the flag.
...
### `docs.aipro.ro` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.aipro.ro` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.aipro.ro` to the allowlist, then retry.
...
- Check the allowlist: `aipro config get agents.defaults.models`
...
If you can, include the relevant log tail from `aipro logs --follow`.
