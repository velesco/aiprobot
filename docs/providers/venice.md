---
summary: "Use Venice AI privacy-focused models in AIPro"
read_when:
  - You want privacy-focused inference in AIPro
...
```bash
aipro onboard --auth-choice venice-api-key
```
...
```bash
aipro chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, AIPro shows all available Venice models. Pick based on your needs:
...
```bash
aipro models set venice/claude-opus-45
aipro models set venice/llama-3.3-70b
```

List all available models:

```bash
aipro models list | grep venice
```

## Configure via `aipro configure`

1. Run `aipro configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
AIPro automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
aipro chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
aipro chat --model venice/claude-opus-45

# Use uncensored model
aipro chat --model venice/venice-uncensored

# Use vision model with image
aipro chat --model venice/qwen3-vl-235b-a22b

# Use coding model
aipro chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `aipro models list` to see currently available models. Some models may be temporarily offline.
