...
This guide summarizes a sane workflow for working on the pi integration in AIPro.
...
- Trigger the agent directly:
  - `pnpm aipro agent --message "Hello" --thinking low`
...
State lives under the AIPro state directory. Default is `~/.aipro`. If `AIPRO_STATE_DIR` is set, use that directory instead.

To reset everything:

- `aipro.json` for config
...
- https://docs.aipro.ro/testing
- https://docs.aipro.ro/start/getting-started
