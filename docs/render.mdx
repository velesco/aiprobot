...
Deploy AIPro on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a href="https://render.com/deploy?repo=https://github.com/aipro/aipro" target="_blank" rel="noreferrer">Deploy to Render</a>
...
repository configures everything needed to run AIPro:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: DIGIBOSS_STATE_DIR
        value: /data/.aipro
      - key: DIGIBOSS_WORKSPACE_DIR
        value: /data/workspace
      - key: DIGIBOSS_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/aipro`.
...
If you use the original AIPro repository, Render will not auto-deploy your AIPro. To update it, run a manual Blueprint sync from the dashboard.
...
For AIPro, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any AIPro host.
