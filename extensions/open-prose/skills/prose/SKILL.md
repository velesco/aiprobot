...
metadata: {"aipro":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == AIPro `sessions_spawn`
- **File I/O** == AIPro `read`/`write`
- **Remote fetch** == AIPro `web_fetch` (or `exec` with curl when POST is required)
