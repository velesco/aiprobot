...
AIPro extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
aipro channels login --channel zalouser
...
```bash
aipro message send --channel zalouser --target <threadId> --message "Hello from AIPro!"
```
...
```bash
aipro channels login --channel zalouser              # Login via QR
aipro channels login --channel zalouser --account work
aipro channels status --probe
aipro channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
aipro directory self --channel zalouser
aipro directory peers list --channel zalouser --query "name"
aipro directory groups list --channel zalouser --query "work"
aipro directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
aipro message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
aipro message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `aipro channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
aipro channels login --channel zalouser --account work
aipro message send --channel zalouser --account work --target <id> --message "Hello"
