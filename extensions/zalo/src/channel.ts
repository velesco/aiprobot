...
export const zaloDock: ChannelDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveZaloAccount({ cfg: cfg as AIProConfig, accountId }).config.allowFrom ?? []).map(
        (entry) => String(entry),
      ),
...
export const zaloPlugin: ChannelPlugin<ResolvedZaloAccount> = {
  ...
  config: {
    listAccountIds: (cfg) => listZaloAccountIds(cfg as AIProConfig),
    resolveAccount: (cfg, accountId) => resolveZaloAccount({ cfg: cfg as AIProConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultZaloAccountId(cfg as AIProConfig),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as AIProConfig).channels?.zalo?.accounts?.[resolvedAccountId],
      );
  ...
  directory: {
    ...
    listPeers: async ({ cfg, accountId, query, limit }) => {
      const account = resolveZaloAccount({ cfg: cfg as AIProConfig, accountId });
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      const account = resolveZaloAccount({ cfg: cfg as AIProConfig });
