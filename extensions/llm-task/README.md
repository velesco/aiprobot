...
This extension depends on AIPro internal modules (the embedded agent runner).
It is intended to ship as a **bundled** AIPro extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.aipro/extensions` as a standalone plugin directory.
