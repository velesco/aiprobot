...
import { GoogleChatConfigSchema } from "aipro/plugin-sdk";
...
export const googlechatDock: ChannelDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveGoogleChatAccount({ cfg: cfg as AIProConfig, accountId }).config.dm?.allowFrom ??
        []
...
export const googlechatPlugin: ChannelPlugin<ResolvedGoogleChatAccount> = {
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      const account = resolveGoogleChatAccount({ cfg: cfg as AIProConfig });
  ...
  config: {
    listAccountIds: (cfg) => listGoogleChatAccountIds(cfg as AIProConfig),
    resolveAccount: (cfg, accountId) =>
      resolveGoogleChatAccount({ cfg: cfg as AIProConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultGoogleChatAccountId(cfg as AIProConfig),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as AIProConfig).channels?.["googlechat"]?.accounts?.[resolvedAccountId],
      );
    ...
    applyAccountConfig: ({ cfg, accountId, input }) => {
      ...
      const next =
        accountId !== DEFAULT_ACCOUNT_ID
          ? migrateBaseNameToDefaultAccount({
              cfg: namedConfig as AIProConfig,
              channelKey: "googlechat",
            })
          : namedConfig;
