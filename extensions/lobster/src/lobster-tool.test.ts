...
import type { AIProPluginApi, AIProPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "aipro-lobster-plugin-") {
...
function fakeApi(overrides: Partial<AIProPluginApi> = {}): AIProPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<AIProPluginToolContext> = {}): AIProPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: AIProPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
