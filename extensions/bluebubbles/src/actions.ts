...
export const bluebubblesMessageActions: ChannelMessageActionAdapter = {
  listActions: ({ cfg }) => {
    const account = resolveBlueBubblesAccount({ cfg: cfg as AIProConfig });
    if (!account.enabled || !account.configured) return [];
    const gate = createActionGate((cfg as AIProConfig).channels?.bluebubbles?.actions);
  ...
  handleAction: async ({ action, params, cfg, accountId, toolContext }) => {
    ...
    const opts = { cfg: cfg as AIProConfig, accountId: accountId ?? undefined };
