...
function resolveAIProRoot(): string {
  ...
  const override = process.env.AIPRO_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set AIPRO_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveAIProRoot(), "dist", "extensionAPI.js");
