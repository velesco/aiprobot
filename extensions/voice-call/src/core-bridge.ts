...
function resolveAIProRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.AIPRO_ROOT?.trim() || process.env.AIPRO_ROOT?.trim();
  if (override) {
  ...
  throw new Error(
    "Unable to resolve core root. Set AIPRO_ROOT (or legacy AIPRO_ROOT) to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveAIProRoot();
