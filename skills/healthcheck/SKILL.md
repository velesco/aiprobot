---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for AIPro deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, AIPro cron scheduling for periodic checks, or version status checks on a machine running AIPro (laptop, workstation, Pi, VPS).
---

# AIPro Host Hardening

## Overview

Assess and harden the host running AIPro, then align it to a user-defined risk tolerance without breaking access. Use AIPro security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim AIPro changes the host firewall, SSH, or OS updates; it does not.
...
5. AIPro gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if AIPro can access sensitive data.
...
### 2) Run AIPro security audits (read-only)

As part of the default read-only checks, run `aipro security audit --deep`. Only offer alternatives if the user requests them:

1. `aipro security audit` (faster, non-probing)
2. `aipro security audit --json` (structured output)

Offer to apply AIPro safe defaults (numbered):

1. `aipro security audit --fix`

Be explicit that `--fix` only tightens AIPro defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check AIPro version/update status (read-only)

As part of the default read-only checks, run `aipro update status`.
...
- Credential hygiene notes (location of AIPro creds, prefer disk encryption)
...
After AIPro install or first hardening pass, run at least one baseline audit and version check:

- `aipro security audit`
- `aipro security audit --deep`
- `aipro update status`

Ongoing monitoring is recommended. Use the AIPro cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `aipro cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `aipro update status`
...
Before creating, `aipro cron list` and match on exact `name`. If found, `aipro cron edit <id> ...`.
If not found, `aipro cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `aipro update status` (preferred for source checkouts and channels)
2. `npm view aipro version` (published npm version)

## AIPro command accuracy

Use only supported commands and flags:

- `aipro security audit [--deep] [--fix] [--json]`
- `aipro status` / `aipro status --deep`
- `aipro health --json`
- `aipro update status`
- `aipro cron add|list|runs|run`

Do not invent CLI flags or imply AIPro enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by AIPro compaction:
