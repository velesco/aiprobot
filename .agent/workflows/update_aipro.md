---
description: Update AIPro from upstream when branch has diverged (ahead/behind)
---

# AIPro Upstream Sync Workflow
...
# Move old version
mv /Applications/AIPro.app /tmp/AIPro-backup.app

# Install new build
cp -R dist/AIPro.app /Applications/

# Launch
open /Applications/AIPro.app
```
...
ps aux | grep -E "(aipro|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm aipro agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
# Update aipro.json with fallback chains
...
pnpm aipro agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
