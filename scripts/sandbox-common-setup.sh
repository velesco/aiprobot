...
BASE_IMAGE="${BASE_IMAGE:-aipro-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-aipro-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=aipro.sandbox=1)
NOTE
