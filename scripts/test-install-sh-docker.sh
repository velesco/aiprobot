...
SMOKE_IMAGE="${AIPRO_INSTALL_SMOKE_IMAGE:-aipro-install-smoke:local}"
NONROOT_IMAGE="${AIPRO_INSTALL_NONROOT_IMAGE:-aipro-install-nonroot:local}"
INSTALL_URL="${AIPRO_INSTALL_URL:-https://aipro.ro/install.sh}"
CLI_INSTALL_URL="${AIPRO_INSTALL_CLI_URL:-https://aipro.ro/install-cli.sh}"
SKIP_NONROOT="${AIPRO_INSTALL_SMOKE_SKIP_NONROOT:-0}"
LATEST_DIR="$(mktemp -d)"
...
docker run --rm -t \
  ...
  -e AIPRO_INSTALL_SMOKE_PREVIOUS="${AIPRO_INSTALL_SMOKE_PREVIOUS:-}" \
  -e AIPRO_INSTALL_SMOKE_SKIP_PREVIOUS="${AIPRO_INSTALL_SMOKE_SKIP_PREVIOUS:-0}" \
...
if [[ "${AIPRO_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
