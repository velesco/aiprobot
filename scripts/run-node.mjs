...
const compiler = env.AIPRO_TS_COMPILER === "tsc" ? "tsc" : "tsgo";
...
const shouldBuild = () => {
  if (env.AIPRO_FORCE_BUILD === "1") return true;
...
const logRunner = (message) => {
  if (env.AIPRO_RUNNER_LOG === "0") return;
  process.stderr.write(`[aipro] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["aipro.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
