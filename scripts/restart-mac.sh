#!/usr/bin/env bash
# Reset AIPro like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${AIPRO_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="AIPro.app/Contents/MacOS/AIPro"
...
WAIT_FOR_LOCK=0
LOG_PATH="${AIPRO_RESTART_LOG:-/tmp/aipro-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${AIPRO_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.aipro/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  AIPRO_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node aipro.mjs daemon install --force --runtime node"
      log "  node aipro.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.aipro/disable-launchagent"
      log ""
...
log "==> Killing existing AIPro instances"
kill_all_aipro
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product AIPro"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/AIPro.app" ]]; then
    APP_BUNDLE="/Applications/AIPro.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/AIPro.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/AIPro.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/AIPro.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set AIPRO_APP_BUNDLE to your installed AIPro.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node aipro.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node aipro.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".aipro", "aipro.json"), "utf8");
