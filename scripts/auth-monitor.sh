...
#
# Suggested cron: */30 * * * * /home/admin/aipro/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send AIPro notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., aipro-alerts)
...
STATE_FILE="$HOME/.aipro/auth-monitor-state"
...
send_notification() {
    ...
    # Send via AIPro if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use aipro
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via AIPro to $NOTIFY_PHONE..."
            aipro send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! AIPro is down. Run: ssh l36 '~/aipro/scripts/mobile-reauth.sh'" "urgent"
    exit 1
