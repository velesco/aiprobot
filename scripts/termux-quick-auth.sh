...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/aipro/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "AIPro Re-Auth" -c "After getting key, run: ssh $SERVER '~/aipro/scripts/mobile-reauth.sh'" --id aipro-auth
        ;;
    *)
