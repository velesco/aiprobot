...
# Checks both Claude Code and AIPro auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
AIPRO_AUTH="$HOME/.aipro/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    aipro models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    aipro_status=$(check_aipro_auth 2>/dev/null || true)

    claude_expires=0
    aipro_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        aipro_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        aipro_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$AIPRO_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        --arg bs "$aipro_status" \
        --arg be "$aipro_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            aipro: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$aipro_status" == EXPIRED* ]] || [[ "$aipro_status" == MISSING* ]]; then
        echo "AIPRO_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$aipro_status" == EXPIRING* ]]; then
        echo "AIPRO_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "AIPro Auth (~/.aipro/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$AIPRO_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$AIPRO_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'aipro doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active aipro >/dev/null 2>&1; then
    echo -e "AIPro service: ${GREEN}running${NC}"
else
    echo -e "AIPro service: ${RED}NOT running${NC}"
fi
