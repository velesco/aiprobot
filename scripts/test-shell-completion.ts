 ...
 * `aipro update`. Use it to verify the completion installation flow
...
\`aipro update\`, \`aipro doctor\`, and \`aipro onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# AIPro Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`aipro completion --install\` later to enable.`));
    return;
  }
