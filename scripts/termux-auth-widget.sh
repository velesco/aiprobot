...
STATUS=$(ssh "$SERVER" '$HOME/aipro/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/aipro/scripts/claude-auth-status.sh json' 2>&1)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/aipro/scripts/mobile-reauth.sh'"
                ;;
            *)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: AIPro won't work until re-auth"
                ;;
        esac
        ;;
