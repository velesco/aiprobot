...
ZIP=${1:?"Usage: $0 AIPro-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/aipro/aipro/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^AIPro-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/aipro/aipro/releases/download/v${VERSION}/"}
