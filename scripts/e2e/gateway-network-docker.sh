...
IMAGE_NAME="aipro-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="aipro-net-e2e-$$"
GW_NAME="aipro-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "AIPRO_GATEWAY_TOKEN=$TOKEN" \
	  -e "AIPRO_SKIP_CHANNELS=1" \
	  -e "AIPRO_SKIP_GMAIL_WATCHER=1" \
	  -e "AIPRO_SKIP_CRON=1" \
	  -e "AIPRO_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
