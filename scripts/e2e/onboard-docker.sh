...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/aipro-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/aipro-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/aipro-e2e-$1.XXXXXX"
  }
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.aipro/aipro.json" <<'"'"'JSON'"'"'
{
