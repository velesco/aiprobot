#!/bin/bash
# Setup AIPro Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'aipro-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  AIPro message: Send warning via AIPro itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/aipro-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/aipro-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/aipro-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now aipro-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status aipro-auth-monitor.timer"
echo ""
echo "Quick commands:"
