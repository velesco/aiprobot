...
const originalConnect = AIProApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("aipro-app") as AIProApp;
...
beforeEach(() => {
  AIProApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__AIPRO_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  AIProApp.prototype.connect = originalConnect;
  window.__AIPRO_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/aipro/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/aipro");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/aipro/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__AIPRO_CONTROL_UI_BASE_PATH__ = "/aipro";
    const app = mountApp("/aipro/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/aipro");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/aipro/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem("aipro.control.settings.v1", JSON.stringify({ token: "existing-token" }));
    const app = mountApp("/ui/overview?token=abc123");
    await app.updateComplete;
