import type { AIProApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as AIProApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as AIProApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as AIProApp);
  }
  if (host.tab === "agents") {
    const app = host as unknown as AIProApp;
