...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") await loadPresence(host as unknown as AIProApp);
  if (host.tab === "sessions") await loadSessions(host as unknown as AIProApp);
  if (host.tab === "cron") await loadCron(host);
  if (host.tab === "skills") await loadSkills(host as unknown as AIProApp);
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as AIProApp);
    await loadDevices(host as unknown as AIProApp);
    await loadConfig(host as unknown as AIProApp);
    await loadExecApprovals(host as unknown as AIProApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as AIProApp);
    await loadConfig(host as unknown as AIProApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as AIProApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as AIProApp, { reset: true });
    scheduleLogsScroll(
...
export function inferBasePath() {
  ...
  const configured = window.__AIPRO_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as AIProApp, false),
    loadPresence(host as unknown as AIProApp),
    loadSessions(host as unknown as AIProApp),
    loadCronStatus(host as unknown as AIProApp),
    loadDebug(host as unknown as AIProApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as AIProApp, true),
    loadConfigSchema(host as unknown as AIProApp),
    loadConfig(host as unknown as AIProApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as AIProApp, false),
    loadCronStatus(host as unknown as AIProApp),
    loadCronJobs(host as unknown as AIProApp),
  ]);
}
