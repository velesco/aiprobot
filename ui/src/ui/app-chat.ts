...
export async function handleAbortChat(host: ChatHost) {
  ...
  host.chatMessage = "";
  await abortChatRun(host as unknown as AIProApp);
}
...
async function sendChatMessageNow(
  ...
  const ok = await sendChatMessage(host as unknown as AIProApp, message, opts?.attachments);
...
export async function refreshChat(host: ChatHost) {
  await Promise.all([
    loadChatHistory(host as unknown as AIProApp),
    loadSessions(host as unknown as AIProApp, { activeMinutes: 0 }),
