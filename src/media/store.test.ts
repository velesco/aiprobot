...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "AIPRO_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.AIPRO_STATE_DIR = path.join(home, ".aipro");
    ...
    await fs.mkdir(path.join(home, ".aipro"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.aipro${path.sep}media`);
