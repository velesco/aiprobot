...
import { loadAIProPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.AIPRO_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `aipro-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "aipro.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    delete process.env.AIPRO_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.AIPRO_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadAIProPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.AIPRO_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadAIProPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadAIProPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.AIPRO_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
