...
import { discoverAIProPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: AIProPluginDefinition;
  register?: AIProPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as AIProPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as AIProPluginDefinition;
...
export function loadAIProPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverAIProPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "aipro/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: AIProPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as AIProPluginModule;
    } catch (err) {
