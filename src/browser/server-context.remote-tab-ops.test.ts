...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const { createBrowserRouteContext } = await import("./server-context.js");
    const state = makeState("aipro");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const aipro = ctx.forProfile("aipro");

    const opened = await aipro.openTab("https://created.example");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("aipro")?.lastTargetId).toBe("CREATED");
  });
});
