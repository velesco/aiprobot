...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".aipro-profile-decorated");
}
 ...
 */
export function decorateAIProProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_AIPRO_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_AIPRO_BROWSER_COLOR).toUpperCase();
