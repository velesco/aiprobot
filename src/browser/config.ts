import type { BrowserConfig, BrowserProfileConfig, AIProConfig } from "../config/config.js";
...
import {
  DEFAULT_AIPRO_BROWSER_COLOR,
  DEFAULT_AIPRO_BROWSER_ENABLED,
  DEFAULT_BROWSER_EVALUATE_ENABLED,
  DEFAULT_BROWSER_DEFAULT_PROFILE_NAME,
  DEFAULT_AIPRO_BROWSER_PROFILE_NAME,
} from "./constants.js";
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_AIPRO_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "aipro" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_AIPRO_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_AIPRO_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an AIPro browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for aipro/aipro CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: AIProConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_AIPRO_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "aipro";
