...
export function listWhatsAppAuthDirs(cfg: AIProConfig): string[] {
...
export function hasAnyWhatsAppAuth(cfg: AIProConfig): boolean {
  return listWhatsAppAuthDirs(cfg).some((authDir) => hasWebCredsSync(authDir));
}

export function listWhatsAppAccountIds(cfg: AIProConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultWhatsAppAccountId(cfg: AIProConfig): string {
...
export function resolveWhatsAppAuthDir(params: { cfg: AIProConfig; accountId: string }): {
  authDir: string;
  isLegacy: boolean;
} {
...
export function listEnabledWhatsAppAccounts(cfg: AIProConfig): ResolvedWhatsAppAccount[] {
