...
describe("hasAnyWhatsAppAuth", () => {
  ...
  beforeEach(() => {
    previousOauthDir = process.env.AIPRO_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "aipro-oauth-"));
    process.env.AIPRO_OAUTH_DIR = tempOauthDir;
  });
  ...
  afterEach(() => {
    if (previousOauthDir === undefined) {
      ...
      process.env.AIPRO_OAUTH_DIR = previousOauthDir;
    }
    if (tempOauthDir) {
  ...
  it("includes authDir overrides", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "aipro-wa-auth-"));
    try {
