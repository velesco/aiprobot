...
This directory contains hooks that ship with AIPro. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.aipro.ro/hooks/soul-evil
...
Hooks can be configured in `~/.aipro/aipro.json`:
...
2. Restart gateway: `pkill -9 -f 'aipro.*gateway' && pnpm aipro gateway`
3. Enable the hook: `aipro hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.aipro.ro/hooks
