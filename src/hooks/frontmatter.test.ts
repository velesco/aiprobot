...
`;
    ...
    expect(parsed.aipro.emoji).toBe("💾");
    expect(parsed.aipro.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.aipro.emoji).toBe("📝");
    expect(parsed.aipro.events).toEqual(["command"]);
    expect(parsed.aipro.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.aipro.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"aipro": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"aipro": {"events": ["test"]}}');
  });
...
describe("resolveAIProMetadata", () => {
  it("extracts aipro metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveAIProMetadata(frontmatter);
  ...
  it("returns undefined when aipro key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        aipro: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with AIPro" },
            { id: "npm", kind: "npm", package: "@aipro/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@aipro/hook");
  });
...
`;
    ...
    const aipro = resolveAIProMetadata(frontmatter);
    expect(aipro).toBeDefined();
    expect(aipro?.emoji).toBe("💾");
    expect(aipro?.events).toEqual(["command:new"]);
    expect(aipro?.requires?.config).toEqual(["workspace.dir"]);
    expect(aipro?.install?.[0].kind).toBe("bundled");
  });
