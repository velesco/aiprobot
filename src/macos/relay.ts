...
async function main() {
  ...
  const { ensureAIProCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[aipro] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
...
void main().catch((err) => {
  console.error("[aipro] Relay failed:", err instanceof Error ? (err.stack ?? err.message) : err);
  process.exit(1);
});
