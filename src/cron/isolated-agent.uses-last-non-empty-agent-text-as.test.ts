...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "aipro-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".aipro", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<AIProConfig> = {},
): AIProConfig {
  const base: AIProConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".aipro", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
