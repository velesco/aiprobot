...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.AIPRO_OAUTH_DIR;
  });

  it("writes auth-profiles.json under AIPRO_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-oauth-"));
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to AIPRO_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-minimax-"));
    process.env.AIPRO_STATE_DIR = tempStateDir;
    process.env.AIPRO_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.AIPRO_AGENT_DIR;
