...
function resolveDefaultAccountId(cfg: AIProConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: AIProConfig): boolean {
...
export function summarizeBindings(cfg: AIProConfig, bindings: AgentBinding[]): string[] {
