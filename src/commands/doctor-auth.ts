...
function pruneAuthProfiles(
  cfg: AIProConfig,
  profileIds: Set<string>,
): { next: AIProConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("aipro models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("aipro models auth setup-token")} or ${formatCliCommand(
      "aipro configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "aipro models auth login --provider openai-codex",
    )} or ${formatCliCommand("aipro configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("aipro configure")}\` or \`${formatCliCommand("aipro onboard")}\`.`;
}
