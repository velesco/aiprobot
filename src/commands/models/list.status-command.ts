...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveAIProAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("aipro models auth setup-token")}\` or \`${formatCliCommand("aipro configure")}\`.`
          : `Run \`${formatCliCommand("aipro configure")}\` or set an API key env var.`;
