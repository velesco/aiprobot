...
function resolveMode(cfg: AIProConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveAIProPackageRoot({
  ...
  let cfg: AIProConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("aipro configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("aipro config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("aipro setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("aipro doctor --fix")}" to apply changes.`);
  }
