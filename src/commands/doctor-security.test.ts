...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    note.mockClear();
    prevToken = process.env.AIPRO_GATEWAY_TOKEN;
    prevPassword = process.env.AIPRO_GATEWAY_PASSWORD;
  ...
  afterEach(() => {
    ...
    if (prevPassword === undefined) delete process.env.AIPRO_GATEWAY_PASSWORD;
    else process.env.AIPRO_GATEWAY_PASSWORD = prevPassword;
  });

  const lastMessage = () => String(note.mock.calls.at(-1)?.[0] ?? "");

  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as AIProConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.AIPRO_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as AIProConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as AIProConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
