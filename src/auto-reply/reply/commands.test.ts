...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-commands-"));
...
function buildParams(commandBody: string, cfg: AIProConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
