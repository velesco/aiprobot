...
export function listSignalAccountIds(cfg: AIProConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: AIProConfig): string {
...
function mergeSignalAccountConfig(cfg: AIProConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: AIProConfig): ResolvedSignalAccount[] {
