...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  return (
    lower.includes("aipro_service_marker") &&
    lower.includes(GATEWAY_SERVICE_MARKER.toLowerCase()) &&
    lower.includes("aipro_service_kind") &&
...
function isAIProGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ro.aipro.") || label.startsWith("com.aipro.");
}

function isAIProGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) return true;
  if (!name.startsWith("aipro-gateway")) return false;
...
function isIgnoredLaunchdLabel(label: string): boolean {
  return (
    ...
    resolveLegacyGatewayLaunchAgentLabels(process.env.AIPRO_PROFILE).includes(label)
  );
}
...
async function scanLaunchdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (isAIProGatewayLaunchdService(label, contents)) continue;
    results.push({
      platform: "darwin",
      label,
...
async function scanSystemdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (!containsMarker(contents)) continue;
    if (isAIProGatewaySystemdService(name, contents)) continue;
    results.push({
      platform: "linux",
      label: entry,
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (!name) continue;
      if (isAIProGatewayTaskName(name)) continue;
