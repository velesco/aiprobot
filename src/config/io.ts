...
function coerceConfig(value: unknown): AIProConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as AIProConfig;
}
...
function stampConfigVersion(cfg: AIProConfig): AIProConfig {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): AIProConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as AIProConfig, deps.env);
      }
  ...
  async function writeConfigFile(cfg: AIProConfig) {
    clearConfigCache();
...
// module scope. `AIPRO_CONFIG_PATH` (and friends) are expected to work even
...
export function loadConfig(): AIProConfig {
...
export async function writeConfigFile(cfg: AIProConfig): Promise<void> {
  clearConfigCache();
