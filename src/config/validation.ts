...
import type { AIProConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: AIProConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: AIProConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = AIProSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as AIProConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as AIProConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as AIProConfig)),
    ),
  };
}
