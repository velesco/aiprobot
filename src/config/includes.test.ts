...
const ETC_AIPRO_DIR = path.join(ROOT_DIR, "etc", "aipro");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "aipro.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcAIProPath(...parts: string[]) {
  return path.join(ETC_AIPRO_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcAIProPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "aipro.json"))).toEqual({
      shared: true,
    });
  });
});
