...
import { AIProSchema } from "./zod-schema.js";
...
export type ConfigSchema = ReturnType<typeof AIProSchema.toJSONSchema>;
...
const FIELD_HELP: Record<string, string> = {
  "meta.lastTouchedVersion": "Auto-set when AIPro writes the config.",
  ...
  "gateway.controlUi.basePath": "Optional URL prefix where the Control UI is served (e.g. /aipro).",
  ...
  "diagnostics.cacheTrace.filePath":
    "JSONL output path for cache trace logs (default: $AIPRO_STATE_DIR/logs/cache-trace.jsonl).",
  ...
  "agents.defaults.memorySearch.store.path":
    "SQLite index path (default: ~/.aipro/memory/{agentId}.sqlite).",
  ...
  "memory.backend": 'Memory backend ("builtin" for AIPro embeddings, "qmd" for QMD sidecar).',
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `aipro plugins update` to locate install sources).",
  "plugins.installs.*.source": 'Install source ("npm", "archive", or "path").',
  "plugins.installs.*.spec": "Original npm spec used for install (if source is npm).",
  "plugins.installs.*.sourcePath": "Original archive/path used for install (if any).",
  "plugins.installs.*.installPath":
    "Resolved install directory (usually ~/.aipro/extensions/<id>).",
...
const FIELD_PLACEHOLDERS: Record<string, string> = {
  ...
  "gateway.controlUi.basePath": "/aipro",
  "gateway.controlUi.root": "dist/control-ui",
  "gateway.controlUi.allowedOrigins": "https://control.example.com",
  "channels.mattermost.baseUrl": "https://chat.example.com",
  "agents.list[].identity.avatar": "avatars/aipro.png",
};
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = AIProSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "AIProConfig";
