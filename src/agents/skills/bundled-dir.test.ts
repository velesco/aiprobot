...
describe("resolveBundledSkillsDir", () => {
  const originalOverride = process.env.AIPRO_BUNDLED_SKILLS_DIR;

  afterEach(() => {
    if (originalOverride === undefined) {
      delete process.env.AIPRO_BUNDLED_SKILLS_DIR;
    } else {
      process.env.AIPRO_BUNDLED_SKILLS_DIR = originalOverride;
    }
  });
  ...
  it("resolves bundled skills under a flattened dist layout", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "aipro" }));
