...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const previousProfile = process.env.AIPRO_PROFILE;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-test-"));
    process.env.AIPRO_STATE_DIR = stateDir;
    process.env.AIPRO_PROFILE = "isolated";

    try {
      const tool = createAIProTools({
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: aipro --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/aipro" } }\n}\n';
