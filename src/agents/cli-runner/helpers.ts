...
/**
 * Cleanup suspended AIPro CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: AIProConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-cli-images-"));
  const paths: string[] = [];
