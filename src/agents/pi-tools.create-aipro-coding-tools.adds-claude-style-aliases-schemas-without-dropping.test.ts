...
import { createAIProTools } from "./aipro-tools.js";
import { __testing, createAIProCodingTools } from "./pi-tools.js";
import { createSandboxedReadTool } from "./pi-tools.read.js";
import { createBrowserTool } from "./tools/browser-tool.js";

const defaultTools = createAIProCodingTools();

describe("createAIProCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createAIProCodingTools({ messageProvider: "discord" });
  ...
  it("filters session tools for sub-agent sessions by default", () => {
    const tools = createAIProCodingTools({
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "aipro-outside.txt");
