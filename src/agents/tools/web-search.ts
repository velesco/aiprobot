...
type WebSearchConfig = NonNullable<AIProConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: AIProConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("aipro configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.aipro.ro/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://aipro.ro",
      "X-Title": "AIPro Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
