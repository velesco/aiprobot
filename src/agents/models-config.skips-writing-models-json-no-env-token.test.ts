...
const MODELS_CONFIG: AIProConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        ...
        const result = await ensureAIProModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      ...
      await ensureAIProModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveAIProAgentDir(), "models.json");
