...
describe("createAIProCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from aipro read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "aipro-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "aipro-workspace"),
    ...
    const tools = createAIProCodingTools({ sandbox });
