...
describe("buildPairingReply", () => {
  ...
  for (const testCase of cases) {
    it(`formats pairing reply for ${testCase.channel}`, () => {
      ...
      // CLI commands should respect AIPRO_PROFILE when set (most tests run with isolated profile)
      const commandRe = new RegExp(
        `(?:aipro|aipro) --profile isolated pairing approve ${testCase.channel} <code>`,
      );
