...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*aipro-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  ...
  records.push(`_aipro-gw._tcp IN PTR ${instanceLabel}._aipro-gw._tcp`);
  records.push(`${instanceLabel}._aipro-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._aipro-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; aipro-content-hash: ${contentHash}\n${contentBody}`;
}
