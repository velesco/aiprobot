...
describe("session cost usage", () => {
  it("aggregates daily totals with log cost and pricing fallback", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-cost-"));
    ...
    const originalState = process.env.AIPRO_STATE_DIR;
    process.env.AIPRO_STATE_DIR = root;
    try {
  ...
  it("summarizes a single session file", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-cost-session-"));
