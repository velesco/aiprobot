...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "aipro doctor",
        managerScriptArgs(manager, "aipro", ["doctor", "--non-interactive"]),
        gitRoot,
        { AIPRO_UPDATE_IN_PROGRESS: "1" },
      ),
    );
