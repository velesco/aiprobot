...
describe("security audit", () => {
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-security-audit-win-"));
  ...
  it("flags Windows ACLs when Users can read the state dir", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-security-audit-win-open-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "aipro.json");
    await fs.writeFile(configPath, "{}\n", "utf-8");
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: AIProConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    const prevStateDir = process.env.AIPRO_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-security-audit-discord-"));
  ...
  it("does not flag Discord slash commands when dm.allowFrom includes a Discord snowflake id", async () => {
    ...
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "aipro-security-audit-discord-allowfrom-snowflake-"),
    );
    process.env.AIPRO_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("flags Discord slash commands when access-group enforcement is disabled and no users allowlist exists", async () => {
    const prevStateDir = process.env.AIPRO_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-security-audit-discord-open-"));
    process.env.AIPRO_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: AIProConfig = {
  ...
  it("flags Slack slash commands without a channel users allowlist", async () => {
    const prevStateDir = process.env.AIPRO_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-security-audit-slack-"));
    process.env.AIPRO_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: AIProConfig = {
  ...
  it("flags Slack slash commands when access-group enforcement is disabled", async () => {
    const prevStateDir = process.env.AIPRO_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-security-audit-slack-open-"));
    process.env.AIPRO_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: AIProConfig = {
  ...
  it("flags Telegram group commands without a sender allowlist", async () => {
    const prevStateDir = process.env.AIPRO_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-security-audit-telegram-"));
    process.env.AIPRO_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: AIProConfig = {
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: AIProConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    ...
    process.env.AIPRO_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: AIProConfig = {
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.aipro",
      configPath: "/Users/test/Dropbox/.aipro/aipro.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-security-audit-"));
    const stateDir = path.join(tmp, "state");
    ...
    try {
      const cfg: AIProConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "aipro.json"),
      });
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.AIPRO_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.AIPRO_GATEWAY_PASSWORD;

    beforeEach(() => {
      delete process.env.AIPRO_GATEWAY_TOKEN;
      delete process.env.AIPRO_GATEWAY_PASSWORD;
    });
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.AIPRO_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.AIPRO_GATEWAY_PASSWORD;
      } else {
        process.env.AIPRO_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env token over local config token", async () => {
      process.env.AIPRO_GATEWAY_TOKEN = "env-token";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: AIProConfig = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.AIPRO_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: AIProConfig = {
        gateway: {
          mode: "remote",
