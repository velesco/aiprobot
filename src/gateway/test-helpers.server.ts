...
async function setupGatewayTestHome() {
  ...
  previousConfigPath = process.env.AIPRO_CONFIG_PATH;
  previousSkipBrowserControl = process.env.AIPRO_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.AIPRO_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.AIPRO_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-gateway-home-"));
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "aipro-test-"))
    : path.join(tempHome, ".aipro-test");
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.AIPRO_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.AIPRO_GATEWAY_TOKEN = fallbackToken;
  }
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.AIPRO_GATEWAY_PASSWORD;
