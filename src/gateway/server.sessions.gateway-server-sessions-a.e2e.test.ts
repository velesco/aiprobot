...
afterAll(async () => {
  await server.close();
  if (previousToken === undefined) delete process.env.AIPRO_GATEWAY_TOKEN;
  else process.env.AIPRO_GATEWAY_TOKEN = previousToken;
});
...
describe("gateway server sessions", () => {
  ...
  test("lists and patches session store via sessions.* RPC", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-sessions-"));
  ...
  test("sessions.preview returns transcript previews", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-sessions-preview-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
