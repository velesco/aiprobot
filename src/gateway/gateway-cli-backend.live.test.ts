  ...
  it("runs the agent pipeline against the local CLI backend", async () => {
    ...
    try {
      ...
      if (previous.configPath === undefined) {
        ...
        process.env.AIPRO_CONFIG_PATH = previous.configPath;
      }
      if (previous.token === undefined) {
        delete process.env.AIPRO_GATEWAY_TOKEN;
      } else {
        process.env.AIPRO_GATEWAY_TOKEN = previous.token;
      }
      if (previous.skipChannels === undefined) {
        delete process.env.AIPRO_SKIP_CHANNELS;
      } else {
        process.env.AIPRO_SKIP_CHANNELS = previous.skipChannels;
      }
      if (previous.skipGmail === undefined) {
        delete process.env.AIPRO_SKIP_GMAIL_WATCHER;
      } else {
        process.env.AIPRO_SKIP_GMAIL_WATCHER = previous.skipGmail;
      }
      if (previous.skipCron === undefined) {
        delete process.env.AIPRO_SKIP_CRON;
      } else {
        process.env.AIPRO_SKIP_CRON = previous.skipCron;
      }
      if (previous.skipCanvas === undefined) {
        delete process.env.AIPRO_SKIP_CANVAS_HOST;
      } else {
        process.env.AIPRO_SKIP_CANVAS_HOST = previous.skipCanvas;
      }
