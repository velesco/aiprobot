...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.AIPRO_SKIP_CHANNELS;
    prevSkipGmail = process.env.AIPRO_SKIP_GMAIL_WATCHER;
    process.env.AIPRO_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.AIPRO_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.AIPRO_SKIP_GMAIL_WATCHER;
    } else {
      process.env.AIPRO_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
