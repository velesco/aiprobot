...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `aipro-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.AIPRO_CONFIG_PATH = path.join(root, "aipro.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "aipro.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "aipro-gateway-test"),
        ...fileDefaults,
