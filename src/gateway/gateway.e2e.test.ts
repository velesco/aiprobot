...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.AIPRO_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-gw-mock-home-"));
      ...
      process.env.AIPRO_SKIP_BROWSER_CONTROL_SERVER = "1";

      const token = `test-${randomUUID()}`;
      process.env.AIPRO_GATEWAY_TOKEN = token;

      const workspaceDir = path.join(tempHome, "aipro");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.aipro-tool-probe.${nonceA}.txt`);
      ...
      try {
        ...
        process.env.AIPRO_CONFIG_PATH = prev.configPath;
        process.env.AIPRO_GATEWAY_TOKEN = prev.token;
        process.env.AIPRO_SKIP_CHANNELS = prev.skipChannels;
        process.env.AIPRO_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.AIPRO_SKIP_CRON = prev.skipCron;
        process.env.AIPRO_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.AIPRO_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "aipro-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.AIPRO_STATE_DIR = prev.stateDir;
      process.env.AIPRO_CONFIG_PATH = prev.configPath;
      process.env.AIPRO_GATEWAY_TOKEN = prev.token;
      process.env.AIPRO_SKIP_CHANNELS = prev.skipChannels;
      process.env.AIPRO_SKIP_GMAIL_WATCHER = prev.skipGmail;
