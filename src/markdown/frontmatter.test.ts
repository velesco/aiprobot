...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { aipro?: { emoji?: string } };
    expect(parsed.aipro?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { aipro?: { events?: string[] } };
