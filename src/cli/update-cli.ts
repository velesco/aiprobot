...
const STEP_LABELS: Record<string, string> = {
  ...
  "aipro doctor": "Running doctor checks",
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", AIPRO_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`AIPRO_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("AIPro update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating AIPro..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this AIPro install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("aipro doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("aipro gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g aipro@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g aipro@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.AIPRO_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.AIPRO_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("aipro gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("aipro doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("aipro gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("aipro gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `aipro update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
    ...
    if (!hasGit) {
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via AIPRO_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update AIPro to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["aipro update", "Update a source checkout (git)"],
        ["aipro update --channel beta", "Switch to beta channel (git + npm)"],
        ["aipro update --channel dev", "Switch to dev channel (git + npm)"],
        ["aipro update --tag beta", "One-off update to a dist-tag or version"],
        ["aipro update --no-restart", "Update without restarting the service"],
        ["aipro update --json", "Output result as JSON"],
        ["aipro update --yes", "Non-interactive (accept downgrade prompts)"],
        ["aipro update wizard", "Interactive update wizard"],
        ["aipro --update", "Shorthand for aipro update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run aipro update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.aipro.ro/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["aipro update status", "Show channel + version status."],
          ["aipro update status --json", "JSON output."],
          ["aipro update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.aipro.ro/cli/update")}`,
    )
    .action(async (opts) => {
      try {
