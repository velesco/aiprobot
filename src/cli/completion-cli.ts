...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# AIPro Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(aipro completion --shell zsh)
  return (
 ...
 * Returns true if profile has `source <(aipro completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option("--write-state", "Write completion scripts to $AIPRO_STATE_DIR/completions (no stdout)")
...
export async function installCompletion(shell: string, yes: boolean, binName = "aipro") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c aipro -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'aipro gateway' and not 'aipro other gateway', we need to check the sequence.
