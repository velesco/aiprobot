...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.aipro.ro/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + AIPro hooks")
    ...
    .option("--hook-url <url>", "AIPro hook URL")
    .option("--hook-token <token>", "AIPro hook token")
