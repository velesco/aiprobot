...
${formatHelpExamples([
  ['aipro agent --to +15555550123 --message "status update"', "Start a new session."],
  ['aipro agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
  ...
  ['aipro agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.aipro.ro/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.aipro.ro/cli/agents")}\n`,
    );
